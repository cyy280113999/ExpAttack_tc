    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            if i == 0 or self.update_gradient:
                if self.aggregate_gradient:
                    # mean of gradients
                    ag = []
                    for j in range(30):
                        # random input
                        if self.gaussian_noise:
                            ran = adv_images + 0.3 * 16/255 * torch.randn_like(adv_images)  # gaussian
                            ran = np.clip(ran,0,1)
                        else:
                            mask = torch.tensor(np.random.binomial(1, 0.9, size=images.shape)).cuda()  # binomial
                            ran = adv_images * mask
                    ...
                    pred_fun(adv_images).sum().backward()
                    g = self.layer.gradient  # g from auto hook
